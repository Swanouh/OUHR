#' Add Plot to Excel
#'
#' Adding a R plot to Excel workbook. Plot can be made using `ggplot` or base
#' `R`. Ensure plot exists in the global environment before using the function.
#'
#' @param wb A string containing the current workbook.
#' @param sheetname A string containing the sheetname.
#' @param plot A plot.
#' @param plot_position A vector of coordinates of starting column and starting
#' row. Defaults to column `2`, row `1`.
...
plot_excel <- function(wb, sheetname, plot, plot_position = c(2, 1),
