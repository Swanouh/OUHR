...
#'
#' Depending on the type specified, creates an email to send link to data or
#' password to the requester.
#'
#' @param To A string of email addresses separated by "`;`".
#' @param Cc A string of email addresses separated by "`;`".
#' @param type Either "`link`", "`password`", "`attachment`" or "`table`" to
#' determine email contents.
#' @param INF_Number A string.
#' @param INF_Name A string.
#' @param dear A string.
#' @param password A string.
#' @param filename A string containing filepath and name of attachment using
#' `/` instead of `\`.
...
email_info_request <- function(To, Cc, type, INF_Number = NULL, INF_Name = NULL,
                               dear = NULL, password = NULL, filename = NULL,
                               table = NULL) {
  ...
  # Body, subject and attachments of message depends on the type of email.
    # PID info requests will have one email to send out the link to the data and
    # another to send out the password.
    # Non PID info requests can either have an email to send out an attachment
    # or an email to send out a table in the body of the email.
  if(type == "link"){
